"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.KelasController = void 0;
const auth_guard_1 = require("../../common/guard/auth.guard");
const admin_guard_1 = require("../../common/guard/admin.guard");
const kelas_service_1 = require("./kelas.service");
const kelas_dto_1 = require("./dto/kelas.dto");
const swagger_1 = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
let KelasController = class KelasController {
    constructor(kelasService) {
        this.kelasService = kelasService;
    }
    async create(req) {
        return this.kelasService.create(req);
    }
    async findAll() {
        return this.kelasService.findAll();
    }
    async findById(kelasId) {
        return this.kelasService.findById(kelasId);
    }
    async update(kelasId, req) {
        return this.kelasService.update(kelasId, req);
    }
    async delete(kelasId) {
        return this.kelasService.delete(kelasId);
    }
};
exports.KelasController = KelasController;
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({
        summary: 'Create Kelas',
    }),
    (0, swagger_1.ApiSecurity)('bearer'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard, admin_guard_1.AdminGuard),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [kelas_dto_1.KelasRequest]),
    __metadata("design:returntype", Promise)
], KelasController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({
        summary: 'Get All Data Kelas',
    }),
    (0, swagger_1.ApiSecurity)('bearer'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard, admin_guard_1.AdminGuard),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], KelasController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)('/:kelasId'),
    (0, swagger_1.ApiOperation)({
        summary: 'Get Data Kelas By ID',
    }),
    (0, swagger_1.ApiSecurity)('bearer'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard, admin_guard_1.AdminGuard),
    __param(0, (0, common_1.Param)('kelasId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], KelasController.prototype, "findById", null);
__decorate([
    (0, common_1.Patch)('/:kelasId'),
    (0, swagger_1.ApiOperation)({
        summary: 'Update Kelas',
    }),
    (0, swagger_1.ApiSecurity)('bearer'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard, admin_guard_1.AdminGuard),
    __param(0, (0, common_1.Param)('kelasId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, kelas_dto_1.KelasRequest]),
    __metadata("design:returntype", Promise)
], KelasController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)('/:kelasId'),
    (0, swagger_1.ApiOperation)({
        summary: 'Delete Kelas',
    }),
    (0, swagger_1.ApiSecurity)('bearer'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard, admin_guard_1.AdminGuard),
    __param(0, (0, common_1.Param)('kelasId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], KelasController.prototype, "delete", null);
exports.KelasController = KelasController = __decorate([
    (0, swagger_1.ApiTags)('Kelas'),
    (0, common_1.Controller)('/api/kelas'),
    __metadata("design:paramtypes", [kelas_service_1.KelasService])
], KelasController);
//# sourceMappingURL=kelas.controller.js.map